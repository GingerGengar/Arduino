...
int LSM9DS1Class::begin()
{
  ...
  writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG1_G, 0xd8); // 952 Hz, 2000dps, 33hz BW
...
void LSM9DS1Class::getIMU(float& ax, float& ay, float& az, float& gx, float& gy, float&gz)
{
  int16_t data[3];

  while (!(readRegister(LSM9DS1_ADDRESS, LSM9DS1_STATUS_REG) & 0x03)){} //poll until both gyro and accel are available

  readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_G, (uint8_t*)data, sizeof(data)); //read gyro
  gx = data[0] * 2000.0/32768.0;
  gy = data[1] * 2000.0/32768.0;
  gz = data[2] * 2000.0/32768.0;

  readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_XL, (uint8_t*)data, sizeof(data)); //read accel
  ax = data[0] * 8.0 / 32768.0;
  ay = data[1] * 8.0 / 32768.0;
  az = data[2] * 8.0 / 32768.0;

}

void LSM9DS1Class::readIMU(float& ax, float& ay, float& az, float& gx, float& gy, float&gz)
{
  int16_t data[3];
...
void LSM9DS1Class::burstReadIMU(float& ax, float& ay, float& az, float& gx, float& gy, float& gz)
{
  int16_t data[11];
  
  readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_G, (uint8_t*)data, sizeof(data)); //read all at once

  gx = data[0] * 2000.0 / 32768.0;
  gy = data[1] * 2000.0 / 32768.0;
  gz = data[2] * 2000.0 / 32768.0;

  ax = data[8] * 8.0/32768.0;
  ay = data[9] * 8.0/32768.0;
  az = data[10] * 8.0/32768.0;
